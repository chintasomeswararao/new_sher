...
import type { ConsoleOutput, ConsoleOutputContent, UIBlock, CustomDocument } from './block';
...
export function PureRunCodeButton({
  ...
  const codeContent = useBlockSelector((state) => {
    if (state.type === 'document' && typeof state.content === 'object' && 'content' in state.content) {
      return (state.content as CustomDocument).content;
    }
    return '';
  });
